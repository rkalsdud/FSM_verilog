...
### shift register
- 신호의 반전  
  테스트 벤치 파일에서는 입력과 클럭을 반전시켜 가며 모듈의 동작을 확인했다.
  
- 반전 주기  
  입력과 클럭의 반전 주기는 서로소로 설정하였다. 그렇지 않았다면 서로 다른 입력과 클럭의 경우를 관측하지 못하고, 같은 조합만이 되풀이 될 것이다. (ex. 클럭 high & 입력 high) 
...
### shift register
 - 시뮬레이션 결과  
   클럭이 상승 엣지인 순간에만 Q의 각 비트가 한 자리씩 왼쪽으로 밀리고, LSB가 해당 시점의 입력 x값으로 채워지는 것을 확인할 수 있었다.

- Moore 머신과 Mealy 머신에서의 차이점  
  - Moore  
    Q의 값이 10101이 되고 나서 다음 클럭 상승 엣지 시점에 출력 z가 활성화됐다. 
  - Mealy  
...
- FPGA 동작 결과  
  ...
  - z 출력  
    Moore 머신에서는 배열 출력이 10101이 되고나서 한 시점 이후에 z 출력이 활성화되었다.  
    Mealy 머신에서는 배열 출력이 1010이면서 입력이 활성화 되는 시점에 z 출력이 활성화되었다. 
